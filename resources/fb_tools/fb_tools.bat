...
@"%php%" "%phpopt%" -r"eval(preg_replace('/^[@:].*$(?=\C*<\?)|^<\?\w*|^\?>\s*$/m','',file_get_contents($_SERVER[argv][0]=$argv[0]=getenv(1))));if(strchr(getenv(0),chr(34))and!strpos(getenv(PSModulePath),getenv(USERNAME)))passthru(pause);" --%argv%
@goto:eof
<?php $ver = "fb_tools 0.22 (c) 11.04.2017 by Michael Engelke <http://www.mengelke.de>"; #(charset=iso-8859-1 / tabs=8 / lines=cr+lf)
